// Task 1 C-Programming Assignment
// Student Id : 2358900
// Name : Subekshya Dhamala
...
// Function to read data from file and accumulate sums
void read_data(FILE *fptr, float *sum_X, float *sum_Y, float *sum_XY, float *sum_Xsq, int *n);

int main() {
    // File pointers for four datasets
    FILE *fptr[4];
    ...
    // Loop through files, read data, and accumulate sums
    for (i = 0; i < 4; i++)
        read_data(fptr[i], &sum_X, &sum_Y, &sum_XY, &sum_Xsq, &n);

    // Calculate linear regression coefficients using math functions
    ...
    // Prompt user for input and predict y
